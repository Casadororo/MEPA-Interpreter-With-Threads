...
void insereInstr_MV_mepa(char *rotulo, instrucao_mepa instr, int op1, int op2,
                         int op3, int op4, char *rotDdesvio) {
...
void relocaDesvios_MV_mepa() {
  ...
  for (i = 0; i < mv_mepa_tam_i;
       i++) { // para cada instrução do Vetor de Instrucoes,
    if (strlen(mv_mepa_I[i].rotDdesvio) !=
        0) { // esta instr tem rotulo de desvio?
      for (j = 0; j < max_tab_reloc;
           j++) { // procura pelo rotulo na tabela de desvios
